...
function calendar_set_date(set_date)
{
    ...
    if(calendar_obj)
    {
        ...
        $("<input type='hidden' id='start_date' name='start_date' value=''>").appendTo(td_id);
        $("<input type='button' name='달력' value='▼' onClick=\"calendar(event, 'start_date', '"+td_id+"');\">").appendTo(td_id);
        //////////////////////////////////////////////////////////////////////////////////
    }
    calendar_close();
	...
	for(var i=1 ; i<check.length ; i++){
		...
		if($(check[i]).is(":checked")) {
			...
			//해당 열에 내용이 있고, 내용과 같은 버튼이 클릭됨
				...
				$(td_check).removeClass(status_eng);
				$(td_check).text("");
				$(td_check).val(-2);
			}
			else if(($(td_check).val() != -2) && ($(td_check).val() != status_val)){
...
function calculate(td_check, btn_val, is_remove){
	...
	if(is_remove == 0){
		//잔여점수 계산
		$(rest).text(parseFloat($(rest).text()) + btn_val);
		$(rest).val(parseFloat($(rest).val()) + btn_val);	
		$(rest).append("<input type='hidden' id='"+input_rest+"' name='"+input_rest+"' value=''>");
		$("#"+input_rest).val(parseFloat($(rest).val()));
		...
		case 0:
			//출석 버튼, 출석 횟수		
			$(present).text(parseInt($(present).text()) + 1); 
			$(present).val(parseInt($(present).val()) + 1);
			$(present).append("<input type='hidden' id='"+input_present+"' name='"+input_present+"' value=''>");	
			$("#"+input_present).val(parseInt($(present).val()));
			break;
		case -0.5:
			//지각 버튼, 지각 횟수
			$(late).text(parseInt($(late).text()) + 1); 
			$(late).val(parseInt($(late).val()) + 1);
			$(late).append("<input type='hidden' id='"+input_late+"' name='"+input_late+"' value=''>");
			$("#"+input_late).val(parseInt($(late).val()));
			break;
		case -1:
			//결석 버튼, 결석 횟수
			$(absent).text(parseInt($(absent).text()) + 1);
			$(absent).val(parseInt($(absent).val()) + 1);
			$(absent).append("<input type='hidden' id='"+input_absent+"' name='"+input_absent+"' value=''>");
			$("#"+input_absent).val(parseInt($(absent).val()));
			break;
		case 1:
			//가산 버튼, 가산 횟수
			$(plus).text(parseInt($(plus).text()) + 1); 
			$(plus).val(parseInt($(plus).val()) + 1);
			$(plus).append("<input type='hidden' id='"+input_plus+"' name='"+input_plus+"' value=''>");
			$("#"+input_plus).val(parseInt($(plus).val()));
			break;
		default:
			break;
		}
	...
	//내용삭제
		...
		case -0.5:
			...
			$(late).val(parseInt($(late).val()) - 1);
			$(late).append("<input type='hidden' id='"+input_late+"' name='"+input_late+"' value=''>");
			$("#"+input_late).val(parseInt($(present).val()));
			break;
		case -1:
			//결석 버튼, 결석 횟수
			$(absent).text(parseInt($(absent).text()) - 1);
